...
SOLUTIONS_HELP_MSG = f"""
    ...
    2. Call heatmaps solution
        yolo solutions heatmap colormap=cv2.COLORMAP_PARULA model=normal.pt

    3. Call queue management solution
        yolo solutions queue region=[(20, 400), (1080, 400), (1080, 360), (20, 360)] model=normal.pt
...
CLI_HELP_MSG = f"""
    ...
    3. Val a pretrained detection model at batch-size 1 and image size 640:
        yolo val model=normal.pt data=coco8.yaml batch=1 imgsz=640
...
def handle_yolo_solutions(args: List[str]) -> None:
    ...
    Examples:
        ...
        >>> handle_yolo_solutions(["inference", "model=normal.pt"])
...
def parse_key_value_pair(pair: str = "key=value"):
    ...
    Examples:
        >>> key, value = parse_key_value_pair("model=normal.pt")
        >>> print(f"Key: {key}, Value: {value}")
        Key: model, Value: normal.pt
...
def entrypoint(debug=""):
    ...
    Examples:
        ...
        >>> entrypoint("train data=coco8.yaml model=normal.pt epochs=10 lr0=0.01")

        Predict a YouTube video using a pretrained segmentation model at image size 320:
        >>> entrypoint("predict model=yolo11n-seg.pt source='https://youtu.be/LNwODJXcvt4' imgsz=320")

        Validate a pretrained detection model at batch-size 1 and image size 640:
        >>> entrypoint("val model=normal.pt data=coco8.yaml batch=1 imgsz=640")
...
if __name__ == "__main__":
    # Example: entrypoint(debug='yolo predict model=normal.pt')
    entrypoint(debug="")
