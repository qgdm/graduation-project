...
class DeepSort(object):
    ...
    #         if len(outputs) > 0:
    #             outputs = np.stack(outputs, axis=0)
    #             with open("deep_sort_results.txt", 'a') as f:
    #                 for output in outputs:
    #                     x1, y1, x2, y2, track_id, track_oid = output
    #                     width = x2 - x1
    #                     height = y2 - y1
    #
    #                     # 保存为MOT格式：[frame, id, x, y, w, h, conf, class, visibility]
    #                     f.write(f"{frame_id},{track_id},{x1},{y1},{width},{height},1,{track_oid},1\n")
    #                     print({frame_id}, {track_id}, {x1}, {y1}, {width}, {height}, 1, {track_oid}, 1)
    #         return outputs
