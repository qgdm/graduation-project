...
def test_major_solutions():
    ...
    counter = solutions.ObjectCounter(region=region_points, model="normal.pt", show=False)  # Test object counter
    heatmap = solutions.Heatmap(colormap=cv2.COLORMAP_PARULA, model="normal.pt", show=False)  # Test heatmaps
    heatmap_count = solutions.Heatmap(
        colormap=cv2.COLORMAP_PARULA, model="normal.pt", show=False, region=region_points
    )  # Test heatmaps with object counting
    speed = solutions.SpeedEstimator(region=region_points, model="normal.pt", show=False)  # Test queue manager
    queue = solutions.QueueManager(region=region_points, model="normal.pt", show=False)  # Test speed estimation
    line_analytics = solutions.Analytics(analytics_type="line", model="normal.pt", show=False)  # line analytics
    pie_analytics = solutions.Analytics(analytics_type="pie", model="normal.pt", show=False)  # line analytics
    bar_analytics = solutions.Analytics(analytics_type="bar", model="normal.pt", show=False)  # line analytics
    area_analytics = solutions.Analytics(analytics_type="area", model="normal.pt", show=False)  # line analytics
    trackzone = solutions.TrackZone(region=region_points, model="normal.pt", show=False)  # Test trackzone
